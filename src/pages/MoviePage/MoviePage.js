...
const MoviePage = () => {
    ...
    async function fetchMovie() {
      try {
        const { data } = await axios.get(
          `https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.REACT_APP_TMDB_API_KEY}&append_to_response=videos,credits,similar`);
        setMovie(data);
      } catch (err) {
        alert('Error');
      }
  ...
  const genres = movie.genres.map((genre) => <NavLink key={genre.id}
                                                      to={`/${genre.name.toLowerCase()
                                                        .replace(/\s+/g, '-')}`}
  >{genre.name.split(' ').join(',')}</NavLink>);
      ...
      <div className={style.wrapper}>
        ...
        >
          <div className={style.info}>
            <h1>{movie.title}</h1>
            <p>{movie.release_date.substring(0, 4)} - {movie.production_countries.map((country) => country.iso_3166_1)
              .join(', ')} - {movie.runtime} min</p>

            <p><span>Genres:</span> <span className={style.genres}>{genres} </span></p>
